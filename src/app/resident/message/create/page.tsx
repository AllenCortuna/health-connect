...
const CreateMessage = () => {
  ...
  const [messageType, setMessageType] = useState<'admin' | 'bhw'>('admin')
  ...
  const searchUsers = useCallback(async (searchQuery: string) => {
    ...
    try {
      ...
      if (messageType === 'admin') {
        // Search admins from accounts collection
        ...
        accountsSnapshot.forEach((doc) => {
          ...
          if (data.role === 'admin' && data.name && data.name.toLowerCase().includes(searchQuery.toLowerCase())) {
            results.push({
              id: doc.id,
              fullName: data.name,
              type: 'admin',
        ...
        <div className="card bg-base-100 shadow-lg">
          <div className="card-body">
              ...
              <div className="flex gap-4">
                <label className="label cursor-pointer">
                  <input
                    ...
                    name="messageType"
                    value="admin"
                    checked={messageType === 'admin'}
                    onChange={(e) => setMessageType(e.target.value as 'admin' | 'bhw')}
            ...
            <div className="form-control">
              <label className="label">
                <span className="label-text font-semibold">Search {messageType === 'admin' ? 'Admin' : 'BHW'}</span>
              </label>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-sm opacity-80">
                      {selectedRecipient.type === 'admin' ? 'Admin' : 'BHW'}
                    </div>
                  </div>
                  <button
                    type="button"
