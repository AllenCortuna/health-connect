...
import { Medicine } from '@/interface/data'
...
const AddMedicine = () => {
  ...
  const [formData, setFormData] = useState<Omit<Partial<Medicine>, 'expDate' | 'createdAt' | 'updatedAt'> & { expDate: string }>({
    medCode: '',
    quantity: 0,
    expDate: new Date(Date.now() + 365 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], // Default to 1 year from now
    name: '',
    description: '',
    medType: 'tablet',
    category: '',
